...
In addition, due to the datasets connection to R.A Fischer there are ties to the ethically controversial field of eugenics. However, even crictics of the dataset acknowledge that it "remains ubiquitous in statistics" and data analysis over 86 years after it's first use [^], with the dataset often referred to as the "hello world" of the Data Science and Machine Learning fields. 

The dataset itself consists of measurements for 3 different classes (setosa, versicolor and virginica) of Iris flowers. 
...
In addition to the categorical variable of the class of Iris, the dataset also records entries for the characteristics of the petal and sepal of the flower, tracking the petal length, petal width, sepal length and sepal width. These four varaibles are numeric decimal place values. 
With this, the data type used for this variables will be float.

While the petal width and length may be a commonly understood variable, the sepal characteristics may be less intuative for users, the below image labels petal and sepal to help understand the data being discussed.

![Labelled Iris flower](Images_for_readme/iris_labelled.png.png)
...
Next we'll take a look at the function for the creation of the summary file write_summary_file().
...
In the above, the FILENAME is first defined to be used for the error handling to ensuring the file is not already existing when the program attempts to create it.  
...
When the if statement condition is met, it is executed and the correlation is calculated using the class defined in the parent "for" loop and the x and y value defined in the second for loop. The name is then created using these values to allow for indexing into the dictionary when creating the table. The name and calculated correlation are then saved as a keypair to the correlations dictionary.
...
To make the program easily readable for the user the code outside of the function is kept to a minimum, for the creation of the summary file the relevant components in the main code block can be seen above. 
...
1. Line triggering the write summary file - this calls the function to be initiated and passes the list and dictionary to the function to be used within the function. 
...
An if statement checks if the iteration is matching one of the numbers in the first condition, if it does the corresponding plots will not contain a scatter plot but instead an empty plot is generated. The figtext method is then used to position the name of a variable within the boundaries of the empty plot, this is to reflect which variable is being used for the x axis in a column and the y axis in the row.  
...
1. Line triggering the create_scatter_all_variables() - this calls the function to be initiated and passes the list and dictionary to be used within the function.
...
The next section will handle the creation of figures containing 2 subplots, showing the distribution of each variable for the entire dataste and then factoring in the class of Iris.
...
1. Line triggering the create_histograms() function - this calls the function to be initiated and passes the dictionary to be used within the function. 
...
Like the other plots, a check is done to ensure the file is not already existing using ![check_png_file_exists()](https://github.com/Ange-Dvs/pands-project/blob/main/README.md#error-handling-for-png-files) and the user is informed.  
...
The same approach is carried out to find the max for the range, these two numbers are then used to generate an array of evenly spaced numbers using NumPy's linespace function.  
...
After the set_bins_width() function completes, it returns that values to be used.

The next steps involves a "for" loop taking the values from the dfs_to_use list and setting them to the variables class_name and class_df.  
...
Like the other plots, a check is done to ensure the file is not already existing ![check_png_file_exists()](https://github.com/Ange-Dvs/pands-project/blob/main/README.md#error-handling-for-png-files) and informs the user if this is the case.  
...
Al label is added to the x-axis of each suplot to identify which variable is being shown in a specific plot. 
...
***
Now that the steps taken in *analysis.py* to create the different files have been explained, lets look what can be understood from information provided in the generated text file and figures. 
...
When looking at the calculated correlation between the variables in the dataset as va whole we can see that some pairs of variables appear to be strongly linked. 
...
The result is indicating an extremely week negative relationship, meaning it could not be assumed that as variable increases the would decrease in a proportionate way.  
...
We can also see that each class of Iris has it's own pattern and relationship between the variables, indicating that the class of species has a big effect on the petal and sepal sizes. 

These differences highlight the importance of understanding the complexity of the data being analysised in the dataset and the importance of considering underlying factors when interpreting the results.  
...
Setosas appear to be more of an outlier and the easiest of the classes to distinguish using the characteristics of the sepal and petal, with Versicolor and Virginica being more closely grouped across all plots.
...
The scatter plot also helps to visulise the correlation previously discussed for each of the variables. 
...
Moving next to the histograms we'll start by looking at a highlevel summary showing a histogram per variable for the dataset as a whole.
...
From a quick look at the above, we can see that the classes overlap for some variables more that others. With some appearing to show more similarities in size while others having distinct clusters depending on the class. 
...
For Versicolor we can see the histogram is a unimodal struction, while not as severe as the Setosa, it has one peak. With Virginica class, we do not have as drastic of a difference between the bins. 
...
Looking at the dataset as a whole, we see the histogram is multimodal with multiple peaks in the plot suggesting that there is a lot of variablity in petal widths across the dataset.
...
Similarily to the petal length, we see that the Setosa data is clustered to the smaller end of the x-axis between (0.1cm to 0.6cm), with this clear seperation of the petal width and length these characteristics could be a good candidate if trying to use the data to identify if an Iris is of the Setosa class.

With the Versicolor and Virginica classes, we see the again slight overlap in the ranges. The histogram for the Versicolor class is unimodal with a clear peak in the distribution and lies again between the Setosa and Virginica classes in terms of size.  
...
While there are many intrinsic factors which could contribute to this similarity between the Versicolor and Virginica class, it has been noted that botanist Edgar Anderson (who provided the data for Fischer to use) believed that the versicolor class could be the "result of hybridisation between the virginica and another class of Iris" [^]. This adds another perspective as to why we see the similarities between the two classes, also acting as reminder that while we can analyise the information contained within the dataset, there is nuance and further context that cannot be reflected.

The range for Versicolor, like for the petal width and length, appears to be in the middle of the Setosa and Virginica clusters. Virginica again seeming to contriubute mostly to the right end of the plot with a larger sepal length. 
...
This histogram appears to be the closest of the variables to match the typical normal dsitribution shape, with one main peak around the mean (3.05cm).
...
Versicolor in comparision is making up the left tail of the distribution, like the Setosa class the Virginica data also appears to be unimodal with one peak. 
...
This finding is in line with the typical suggestion from those who have previously analysied the famous dataset with acidemics such as Maurice Maeck stating the petal "parameters are much more discrimminatory than the sepal parameters" in his 2018 paper *Fisher's iris classification revisited*[^].

## Conclusion
From the analysis of the Iris dataset, we have been able to gain insight into the characteristics of the different Iris classes. Seeing how each class has it’s own patterns across the variables and how some variables may be more suited than others to be used for classification of Irises.
